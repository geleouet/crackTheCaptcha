...
/**
 * 
 * https://github.com/gbaydin/OxCaptcha/
 * 
 * 
 * @author gaeta
 *
...
private static final String ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
    ...
    public static void main(String[] args) throws IOException {
    	...
    	for (String s : ALPHABET.split("")) {
    		if (new File(pathname+"/"+s).exists()) {
        		for (var f:new File(pathname+"/"+s).listFiles()) f.delete();
        	}
        	else {
        		new File(pathname+"/"+s).mkdirs();
        	}
    		for (Font f: allFonts) 
    		{
    			...
    			for (int i = 0; i< nByClass; i++) {
    				...
    				oxCaptcha.distortionElastic(30+RAND.nextInt(16));
    				RAND.setSeed(seed);

    				oxCaptcha.distortionShear(xPeriod, xPhase, yPeriod, yPhase);

    				if (RAND.nextDouble() < 0.2)
    					oxCaptcha.blurGaussian(RAND.nextDouble());


    				oxCaptcha.save(pathname+"/"+s+"/"+cpt.incrementAndGet()+".png");
    			}
    		}
    	}
    ...
    public OxGenerate(int width, int height) {
    ...
    public void text(String chars, Supplier<Integer> kerning) {
        text(chars, (int)(0.05 * _width), (int)(0.75 * _height), kerning);
    }

    public void text(char[] chars) {
        text(chars, (int)(0.05 * _width), (int)(0.75 * _height), () -> 0, () -> 0);
    }

    public void text(String chars, int xOffset, int yOffset, Supplier<Integer> kerning) {
    	text(chars, xOffset, yOffset, () -> 0, kerning);
    }
    public void text(String chars, int xOffset, int yOffset, Supplier<Integer> vkerning, Supplier<Integer> kerning) {
        int l = chars.length();
        char[] t = new char[l];
        chars.getChars(0, l, t, 0);
        text(t, xOffset, yOffset, vkerning, kerning);
    }

    public void text(char[] chars, int xOffset, int yOffset, Supplier<Integer> vkerning, Supplier<Integer> kerning) {
        text(chars, xOffset, yOffset, () -> Font.PLAIN, vkerning, kerning);
    }
    
    public void text(String chars, int xOffset, int yOffset, Supplier<Integer> style, Supplier<Integer> vkerning, Supplier<Integer> kerning) {
        int l = chars.length();
        char[] t = new char[l];
        chars.getChars(0, l, t, 0);
        ...
        text(t, styles, xOffset, yOffset, vkerning, kerning);
    }
    
    public void text(char[] chars, int xOffset, int yOffset, Supplier<Integer> style, Supplier<Integer> vkerning, Supplier<Integer> kerning) {
        int styles[] = new int[chars.length];
        for (int i = 0 ; i < chars.length; i++) {
            styles[i] = style.get();
        }
        text(chars, styles, xOffset, yOffset, vkerning, kerning);
    }
        
    public void text(char[] chars, int[] styles, int xOffset, int yOffset, Supplier<Integer> vkerning, Supplier<Integer> kerning) {
    ...
    public void noiseBig() {
    	noiseBig(0.01f, 0.01f, 0.3f);
    }
    public void noiseBig(float g) {
    	noiseBig(0.01f, 0.01f, g);
    }
    ...
    public void noiseBig(float salt, float pepper, float g) {
    	int s = (int) (_height * _width * salt);
    	int p = (int) (_height * _width * pepper);
...
//        }
    ...
    public void distortionElectric2(double alpha) {
        ...
        dxField = OxGenerate.gaussian(dxField, 2, 2.2);
        dyField = OxGenerate.gaussian(dyField, 2, 2.2);
...
//                    System.out.println(target);
    ...
    public void distortionElastic(double alpha, int kernelSize, double sigma) {
       // RAND.setSeed(3);
        ...
        dxField = OxGenerate.gaussian(dxField, kernelSize, sigma);
        dyField = OxGenerate.gaussian(dyField, kernelSize, sigma);
